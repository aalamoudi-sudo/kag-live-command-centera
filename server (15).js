...
const server=http.createServer(async (req,res)=>{
  try{
    ...
    if(url==="/api/state"){
      // POST من الواجهة: البيانات تأتي من Google Sheet فقط، لا كتابة مدعومة — نرد 200 هادئ
      if(req.method==="POST") return sendJson(res,200,{ok:true,version:liveVersion,updatedAt:liveUpdatedAt});
