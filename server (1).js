...
/* ===== التحقق من القوالب وتحميلها إذا كانت ناقصة ===== */
const TEMPLATE_MIN_SIZE = 500 * 1024; // 500KB — أي قالب أصغر من هذا ناقص
const TEMPLATE_URLS = {
  "comprehensive": process.env.TPL_URL_COMPREHENSIVE || "",
  "track-a":       process.env.TPL_URL_TRACK_A       || "",
  "track-b":       process.env.TPL_URL_TRACK_B       || "",
  "track-c":       process.env.TPL_URL_TRACK_C       || "",
  "track-d":       process.env.TPL_URL_TRACK_D       || "",
};

async function ensureTemplates() {
  const tplDir = path.join(__dirname, "templates");
  if (!require("fs").existsSync(tplDir)) require("fs").mkdirSync(tplDir, {recursive:true});

  const names = Object.keys(TEMPLATE_URLS);
  let downloaded = 0;

  for (const name of names) {
    const dest = path.join(tplDir, name + ".pptx");
    const url  = TEMPLATE_URLS[name];

    // تحقق إذا القالب موجود وحجمه كافٍ
    try {
      const stat = require("fs").statSync(dest);
      if (stat.size >= TEMPLATE_MIN_SIZE) {
        console.log(`[templates] ✓ ${name}.pptx (${Math.round(stat.size/1024)}KB)`);
        continue;
      }
      console.log(`[templates] ⚠ ${name}.pptx صغير جداً (${stat.size} bytes) — سيتم إعادة تحميله`);
    } catch(e) {
      console.log(`[templates] ⚠ ${name}.pptx غير موجود`);
    }

    if (!url) {
      console.error(`[templates] ✗ ${name}.pptx — لا يوجد رابط (TPL_URL_${name.toUpperCase().replace('-','_')})`);
      continue;
    }

    try {
      console.log(`[templates] ⬇ تحميل ${name}.pptx ...`);
      const res = await fetch(url);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const buf = Buffer.from(await res.arrayBuffer());
      require("fs").writeFileSync(dest, buf);
      console.log(`[templates] ✅ ${name}.pptx تم تحميله (${Math.round(buf.length/1024)}KB)`);
      downloaded++;
    } catch(e) {
      console.error(`[templates] ✗ فشل تحميل ${name}.pptx:`, e.message);
    }
  }

  if (downloaded > 0) console.log(`[templates] تم تحميل ${downloaded} قوالب`);
}
