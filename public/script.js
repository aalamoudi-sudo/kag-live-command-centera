...
function paHtml(planned, actual){
  ...
  return `<div class="planned-actual-box">
    ...
    <div class="pa-row"><span>الفرق</span><div class="pa-bar"><i style="width:${Math.max(0,Math.min(100,Math.abs(diff)))}%;background:${diff>=0?'#43ee8d':'#ff5e6b'}"></i></div><b class="${diff>=0?'green':'red'}">${diff>0?'+':''}${diff}%</b></div>
  </div>`;
}
  ...
  tracksSummary.innerHTML = `
    <div class="glass panel project-pa-card">
      ...
      <div class="project-pa-grid">
        ...
        <div class="project-pa-metric"><b class="${diff>=0?'green':'red'}">${diff>0?'+':''}${diff}%</b><span>الفرق</span></div>
      </div>
...
function v20RenderIntelligence(){
  ...
  projectHealthText.textContent = `المخطط ${planned}%، الفعلي ${actualVal}%، الفرق ${planDiff>0?"+":""}${planDiff}%، المخاطر ${v20Items("risks").length}، القرارات المفتوحة ${v20OpenDecisions().length}.`;
  ...
  const alerts = [];
  if(planDiff < -10) alerts.push(["انحراف عن الخطة",`الفرق ${planDiff}%.`,"red"]);
...
function v21RenderHomeSummary(){
  ...
  homeStatusText.textContent = status[1] + ` المخطط ${planned}%، الفعلي ${actualVal}%، الفرق ${diff>0?"+":""}${diff}%.`;
  ...
  homePlannedActualBox.innerHTML = `<div class="home-pa-bars">
    ...
    <div class="home-pa-line"><span>الفرق</span><div class="home-pa-track variance"><i style="width:${Math.max(0,Math.min(100,Math.abs(diff)))}%;background:${diff>=0?'#43ee8d':'#ff5e6b'}"></i></div><b class="${diff>=0?'green':'red'}">${diff>0?'+':''}${diff}%</b></div>
  </div>`;
}
