...
function paHtmlTrackBox(planned, actual){
  const {actual: actualVal, ratio: diff} = paComplianceAbs(planned, actual);
  return `<div class="planned-actual-box">
    <div class="pa-row"><span>المخطط</span><div class="pa-bar planned"><i style="width:${Math.max(0,Math.min(100,planned))}%"></i></div><b>${planned}%</b></div>
    <div class="pa-row"><span>الفعلي</span><div class="pa-bar actual"><i style="width:${Math.max(0,Math.min(100,actualVal))}%"></i></div><b>${actualVal}%</b></div>
    <div class="pa-row"><span>الانحراف</span><div class="pa-bar"><i style="width:${Math.max(0,Math.min(100,Math.abs(diff)))}%;background:${diff>=0?'#43ee8d':'#ff5e6b'}"></i></div><b class="${diff>=0?'green':'red'}">${diff>0?'+':''}${diff}%</b></div>
...
function showDetails(type, trackId=null){
  ...
  }else if(type === "tasks-done"){
    title = "تفاصيل المهام المنجزة";
    subtitle = trackId ? "المهام المنجزة بالمسار المحدد" : "جميع المهام المنجزة على مستوى المشروع";
    items = state.items.filter(i=>i.type==="tasks" && ["مكتملة","معتمدة","Completed","Cleared"].includes(i.status) && (!trackId || i.track===trackId));
  }else if(type === "tasks-active"){
    title = "تفاصيل المهام النشطة";
    subtitle = trackId ? "المهام النشطة بالمسار المحدد" : "جميع المهام النشطة على مستوى المشروع";
    items = state.items.filter(i=>i.type==="tasks" && ["قيد التنفيذ","متأخرة","متاخرة","تحت المتابعة","In Progress","Watch","Overdue"].includes(i.status) && (!trackId || i.track===trackId));
  }else if(type === "tasks-notstarted"){
  ...
  if(modal.list){
    const dateSummary=type==="tasks"?PMCScheduleCompleteness.summarize(items):null;
    const dateSummaryHtml=dateSummary&&dateSummary.incomplete
      ? `<div class="schedule-completeness-summary">⚠ ${dateSummary.incomplete} مهام بتواريخ غير مكتملة</div>`
      : "";
    modal.list.innerHTML = items.length ? dateSummaryHtml+items.map(i=>{
    ...
    const dateState=type==="tasks"?PMCScheduleCompleteness.inspectTask(i):null;
    const dateWarning=dateState&&dateState.incomplete
      ? `<span class="schedule-date-warning">⚠ ${dateState.missingStart&&dateState.missingEnd?"تاريخ البداية والنهاية مفقودان":dateState.missingStart?"تاريخ البداية مفقود":"تاريخ النهاية مفقود"}</span>`
      : "";
    return `
  ...
  return `<article class="track-card glass clickable-card" onclick="showDetails('track','${t.id}')" style="--accent:${t.accent};--value:${t.progress}">
    ...
    <div class="track-body">
      ...
      <div class="mini-grid">
        ...
        <div class="mini clickable-kpi" onclick="event.stopPropagation();showDetails('tasks-done','${t.id}')"><b style="color:#43ee8d">${t.done}</b><small>منجزة</small></div>
        <div class="mini clickable-kpi" onclick="event.stopPropagation();showDetails('tasks-active','${t.id}')"><b style="color:#ffc247">${t.active}</b><small>نشطة</small></div>
  ...
  state.tracks.forEach(t=>{
    ...
    el.innerHTML=`<div class="track-dashboard" style="--accent:${t.accent}">
      ...
      <div class="track-kpis">
        ...
        <div class="track-kpi clickable-kpi" onclick="showDetails('tasks-done','${t.id}')"><b>${t.done}</b><small>المهام المنجزة</small></div>
...
function parseItemDate(str){
  if(!str) return null;
  str = String(str).trim();
  // صيغة السنة أولاً (ISO): 2026-06-07 أو 2026/06/07
  let m = str.match(/^(\d{4})[-\/](\d{1,2})[-\/](\d{1,2})$/);
  if(m) return new Date(`${m[1]}-${m[2].padStart(2,"0")}-${m[3].padStart(2,"0")}T00:00:00`);
  // صيغة اليوم أولاً (شائعة بالخليج): 16/07/2026 أو 25-10-2026 أو 25-9-2026
...
function smartImportRows(rows){
  ...
  const hasHeader = rawHeader.some(h=>["track","type","title","owner","status","due","startdate"].includes(h));
  let map = {track:0,type:1,title:2,owner:3,status:4,due:5,startdate:-1};

  if(hasHeader){
    ["track","type","title","owner","status","due","startdate"].forEach(k=>{
  ...
  rows.forEach(r=>{
    const item = {
      ...
      due: r[map.due] || "",
      startDate: map.startdate>=0 ? r[map.startdate] || "" : ""
    };
